...
public class SmartAgent {
	...
	public Movement foiAtingido(double anguloBala) throws Exception {
		...
		return chooseBestAngle(hitAngle);
	}
	
	public Movement chooseBestAngle(int positionShot) {
		...
		if (!positionEscapeList.isEmpty()) {
			for (PositionEscape positionEscape : positionEscapeList) {
				if (positionEscape.getPositionShot() == positionShot) {
		...
		if (!haPosicoesNaoTestadas) {
			bestPositionEscape = getHighResistance(positionShot);
		...
		case 0:
			return new Movement(100, 0, 0, bestPositionEscape.getAnguleEscape());
		case 1:
			return new Movement(100, 0, bestPositionEscape.getAnguleEscape(), 0);
		case 2:
			return new Movement(0, 100, 0, bestPositionEscape.getAnguleEscape());
		case 3:
			return new Movement(0, 100, bestPositionEscape.getAnguleEscape(), 0);
		default:
			return new Movement(100, 0, 0, 0);
		}
	}
	
	public PositionEscape getHighResistance(int positionShot){
		Random random = new Random();
		PositionEscape melhorPosicao = null;
		for (PositionEscape posicaoFuga : positionEscapeList) {
			if (posicaoFuga.getPositionShot() == positionShot) {
				if (melhorPosicao == null) {
					melhorPosicao = posicaoFuga.clone();
				}
				if (posicaoFuga.getTimeOfResistance().doubleValue() > melhorPosicao.getTimeOfResistance().doubleValue()) {
					melhorPosicao = posicaoFuga.clone();
					
				}else if (posicaoFuga.getTimeOfResistance().doubleValue() ==  melhorPosicao.getTimeOfResistance().doubleValue()) {					
